...
function A() {
    ...
    const [menu, setMenu] = useState('모두')
    const [datalist, setDatalist] = useState(data)
    const [so, setSortList] = useState(data)
    ...
    const setMenuFilter = menu => {
      if(menu !== '모두') { // 메뉴
        setDatalist([...data.filter(e => e.menu === menu)])
      }else{
        setDatalist(data)
      }
      setMenu(menu)
    ...
    const setSortFilter = so => {
      if(sortTab == '리뷰순') { // 정렬
        setDatalist([...data.filter(sort((a,b) => (a.review - b.review)))])
      // }else if(sortTab == '좋아요순'){
      //   setDatalist(data.filter(sort((a,b) => (a.like - b.like))))
      // }else if(sortTab == '별순'){
      //   setDatalist(data.filter(sort((a,b) => (a.star - b.star))))
      }else{
        setDatalist(data)
      }
      setMenu(so)
      ...
      return(
        ...
        />
          ...
          */}
          ...
          {/* <View style={[
            styles.itemStatus,
            {backgroundColor: item.status === '배달x' ? '#e5848e': '#69c080'}]}
            >
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        {/* <View style={styles.sortTab}> 
        {
            listTab.map(e => (
              <TouchableOpacity
                style={[styles.btnTab, so === e.so && styles.btnTabActive]}
                //onPress={() => setSortFilter(e)}
              >
                <Text style={styles.textTab}>
                  {e.so}
                </Text>
        ...
        <View style={styles.menuTab}> 
        {
            MenuTab.map(e => (
              <TouchableOpacity
                style={[styles.btnMenuTab, menu === e.menu && styles.btnTabActive]}
                onPress={() => setMenuFilter(e.menu)}
              >
                <Text style={styles.textMenuTab}>
                  {e.menu}
                </Text>
              </TouchableOpacity>
              ))
        ...
        <FlatList
        // data={datalist.sort((a,b) => a.name.localeCompare(b.name))} //는 가나다 순으로 정렬
        // data={ datalist.sort((a,b) => (a.review - b.review))} //는 숫자 정렬
